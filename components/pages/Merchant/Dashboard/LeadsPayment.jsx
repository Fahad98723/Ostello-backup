...
const LeadsPayment = ({
  ...
  useEffect(() => {
    if (couponCode === "OLC2022") {
      setTotal(totalPrice - couponPrice);
    } else {
      setTotal(totalPrice);
    }
  }, [couponCode, couponPrice, totalPrice]);
        ...
        handler: async function (response) {
          ...
          try {
            ...
            const updated = await axios.post(
              `${host}/payments/order/purchasecredits`,
              data,
              {
                headers: {
                  "Access-Control-Allow-Origin": "*",
                      ...
                      <div className="w-[320px] mt-3 rounded-md bg-[#F0ECFA] shadow px-3 py-1">
                        ...
                        <div >
                          <p className="text-[18px]">
                            Apply Coupon Code <small>if you have any</small>
                          </p>
                          <div className="flex items-center w mt-2">
                            <input
                              className="py-3 px-2 rounded w-full"
                              onChange={(e) => setCouponCode(e.target.value)}
                              type="text"
                            />
                            <button
                              onClick={() => couponApply()}
                              className="bg-primary py-3 p-2 ml-2 text-white rounded"
                            >
                              Apply
                            </button>
                          </div>
